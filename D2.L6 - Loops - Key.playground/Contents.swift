...
It will print, both the country and the capital.
 
b. What does the variable pair represent on each iteration?
Pair is the local variable within the for-in loop.

c. What happened when you print pair.key and pair.value?
Printing pair.key prints only the keys, which are the countries.
...
for eachPlace in places2Go {
    print("You are currently \(eachPlace.value) miles away from \(eachPlace.key)")
}
...
for eachPlace in places2Go {
    if eachPlace.value <= 100{
        print("You are currently \(eachPlace.value) miles away from \(eachPlace.key)")
    } else {
        print("WOW! You are \(eachPlace.value) miles away from \(eachPlace.key)")
    }
}
...
for eachWord in arrayOfWords {
    if eachWord.count > 10 {
        largeWords.append(eachWord)
    }
}
...
for eachWord in arrayOfWords {
    var largeWords = [String]()
    var largeWordsNum = [Int]()
    
    if eachWord.count > 10 {
        largeWords.append(eachWord)
        largeWordsNum.append(eachWord.count)
    }
    
    let highestNum = largeWordsNum.max()
        
    if largeWords.count == highestNum {
        largestWord = largeWords
    }
}
*/
func largestWord(words:[String]) -> String {
    ...
    localArray = words
    var localLargeWords = [String]()
    var largeWordsNum = [Int]()
    
    for eachWord in localArray {
        if eachWord.count > 10 {
            localLargeWords.append(eachWord)
            largeWordsNum.append(eachWord.count)
        }
    }
    ...
    for eachWord in localLargeWords {
        let value = eachWord.count
        if value == largeWordsNum.max()! {
            print(eachWord)
            largestWord = eachWord
        }
    }
...
var test = largestWord(words: arrayOfWords)
print(test)
